...
suite("Go Extension Tests", function () {
	...
	suiteSetup(async () => {
		...
		if (!gopath) {
			assert.ok(
				gopath,
				"Cannot run tests if GOPATH is not set as environment variable"
			);
			return;
		}
		...
		generateFunctionTestSourcePath = path.join(
			repoPath,
			"generatefunctiontest"
		);
		generatePackageTestSourcePath = path.join(
			repoPath,
			"generatePackagetest"
		);
		...
		fs.copySync(
			path.join(fixtureSourcePath, "baseTest", "test.go"),
			path.join(fixturePath, "baseTest", "test.go")
		);
		fs.copySync(
	...
	async function testDefinitionProvider(
		...
		const definitionInfo = await provider.provideDefinition(
			textDocument,
			position,
	...
	async function testSignatureHelpProvider(
		...
		const promises = testCases.map(
			([position, expected, expectedDoc, expectedParams]) =>
				provider
					...
					.then((sigHelp) => {
						assert.ok(
							sigHelp,
							`No signature for gogetdocTestData/test.go:${
								position.line + 1
							}:${position.character + 1}`
						);
						assert.equal(
							sigHelp.signatures.length,
							1,
							"unexpected number of overloads"
						);
						...
						assert.equal(
							sigHelp.signatures[0].parameters.length,
							expectedParams.length
						);
						for (let i = 0; i < expectedParams.length; i++) {
							assert.equal(
								sigHelp.signatures[0].parameters[i].label,
								expectedParams[i]
							);
						}
					})
	...
	async function testHoverProvider(
		...
		const promises = testCases.map(
			([position, expectedSignature, expectedDocumentation]) =>
				provider
					.provideHover(
						textDocument,
						position,
					...
					.then((res) => {
						...
						assert.equal(
							(<vscode.MarkdownString>res.contents[0]).value,
							expectedHover
						);
					})
		);
...
`;
	...
	test("Linting - concurrent process cancelation", async () => {
		...
		assert.equal(
			util.runTool.callCount,
			2,
			"should have launched 2 lint jobs"
		);
		assert.equal(
			util.killTree.callCount,
			1,
	...
	test("Test Generate unit tests skeleton for file", async () => {
		...
		const testFileGenerated = fs.existsSync(
			path.join(generateTestsSourcePath, "generatetests_test.go")
		);
	...
	test("Test diffUtils.getEditsFromUnifiedDiffStr", async () => {
		...
		const fileEditPatches: any | FilePatch[] = await new Promise(
			(resolve) => {
				cp.exec(
					`diff -u ${file1path} ${file2path}`,
					(err, stdout, stderr) => {
						...
						if (!filePatches || filePatches.length !== 1) {
							assert.fail(
								null,
								null,
								"Failed to get patches for the test file",
								""
							);
						}
						...
						if (!filePatches[0].fileName) {
							assert.fail(
								null,
								null,
								"Failed to parse the file path from the diff output",
								""
							);
						}
						...
						if (!filePatches[0].edits) {
							assert.fail(
								null,
								null,
								"Failed to parse edits from the diff output",
								""
							);
						}
						resolve(filePatches);
	...
	test("Test Env Variables are passed to Tests", async () => {
		const config = Object.create(vscode.workspace.getConfiguration("go"), {
			testEnvVars: {
				value: { dummyEnvVar: "dummyEnvValue", dummyNonString: 1 },
			},
		});
		const uri = vscode.Uri.file(
			path.join(fixturePath, "baseTest", "sample_test.go")
	...
	test("Test Outline", async () => {
		...
		const outlines = await documentSymbols(
			options,
			dummyCancellationSource.token
		);
		const packageSymbols = outlines.filter(
			(x: any) => x.kind === vscode.SymbolKind.Package
		);
		const imports = outlines[0].children.filter(
			(x: any) => x.kind === vscode.SymbolKind.Namespace
		);
		const functions = outlines[0].children.filter(
			(x: any) => x.kind === vscode.SymbolKind.Function
		);
	...
	test("Test Outline imports only", async () => {
		...
		const packageSymbols = outlines.filter(
			(x) => x.kind === vscode.SymbolKind.Package
		);
		const imports = outlines[0].children.filter(
			(x: any) => x.kind === vscode.SymbolKind.Namespace
		);
	...
	test("Test Outline document symbols", async () => {
		...
		const outlines = await symbolProvider.provideDocumentSymbols(
			document,
			dummyCancellationSource.token
		);
		const packages = outlines.filter(
			(x) => x.kind === vscode.SymbolKind.Package
		);
		const variables = outlines[0].children.filter(
			(x: any) => x.kind === vscode.SymbolKind.Variable
		);
		const functions = outlines[0].children.filter(
			(x: any) => x.kind === vscode.SymbolKind.Function
		);
		const structs = outlines[0].children.filter(
			(x: any) => x.kind === vscode.SymbolKind.Struct
		);
		const interfaces = outlines[0].children.filter(
			(x: any) => x.kind === vscode.SymbolKind.Interface
		);
			...
			if (vendorSupport) {
				vendorPkgsFullPath.forEach((pkg) => {
					assert.equal(
						pkgs.indexOf(pkg) > -1,
						true,
						`Package not found by goPkgs: ${pkg}`
					);
				});
	...
	test("Workspace Symbols", () => {
		...
		const configWithoutIgnoringFolders = Object.create(
		...
		const configWithIgnoringFolders = Object.create(
			vscode.workspace.getConfiguration("go"),
			{
				gotoSymbol: {
					value: {
		...
		const configWithIncludeGoroot = Object.create(
			vscode.workspace.getConfiguration("go"),
			{
				gotoSymbol: {
					value: {
		...
		const configWithoutIncludeGoroot = Object.create(
			vscode.workspace.getConfiguration("go"),
			{
				gotoSymbol: {
					value: {
		...
		const withoutIgnoringFolders = getWorkspaceSymbols(
			...
			assert.equal(
				results[0].path,
				path.join(workspacePath, "vendor/9fans.net/go/acme/acme.go")
			);
		});
		...
		return Promise.all([
			withIgnoringFolders,
			withoutIgnoringFolders,
			withIncludingGoroot,
			withoutIncludingGoroot,
		]);
	});
...
`;
		...
		const promises = testCases.map(
			([position, expectedLabel, expectedDetail, expectedDoc]) =>
				provider
					...
					.then(async (items) => {
						...
						if (!item) {
							assert.fail(
								"missing expected item in completion list"
							);
						}
						assert.equal(item.detail, expectedDetail);
						const resolvedItemResult: vscode.ProviderResult<vscode.CompletionItem> =
							...
							if (resolvedItemResult.documentation) {
								assert.equal(
									(<vscode.MarkdownString>(
										resolvedItemResult.documentation
									)).value,
									expectedDoc
								);
							}
	...
	test("Test Completion Snippets For Functions", async () => {
		...
		const noFunctionSnippet = provider
			...
			.then((items) => {
				items =
					items instanceof vscode.CompletionList
						? items.items
						: items;
				const item = items.find((x) => x.label === "Print");
				if (!item) {
					assert.fail(
						'Suggestion with label "Print" not found in test case noFunctionSnippet.'
					);
				}
		...
		const withFunctionSnippet = provider
			...
			.then((items1) => {
				items1 =
					items1 instanceof vscode.CompletionList
						? items1.items
						: items1;
				const item1 = items1.find((x) => x.label === "Print");
				if (!item1) {
					assert.fail(
						'Suggestion with label "Print" not found in test case withFunctionSnippet.'
					);
				}
				assert.equal(
		...
		const withFunctionSnippetNotype = provider
			...
			.then((items2) => {
				items2 =
					items2 instanceof vscode.CompletionList
						? items2.items
						: items2;
				const item2 = items2.find((x) => x.label === "Print");
				if (!item2) {
					assert.fail(
						'Suggestion with label "Print" not found in test case withFunctionSnippetNotype.'
					);
				}
				assert.equal(
					(<vscode.SnippetString>item2.insertText).value,
					"Print(${1:a})"
				);
			});
		const noFunctionAsVarSnippet = provider
			...
			.then((items3) => {
				items3 =
					items3 instanceof vscode.CompletionList
						? items3.items
						: items3;
				const item3 = items3.find((x) => x.label === "funcAsVariable");
				if (!item3) {
					assert.fail(
						'Suggestion with label "Print" not found in test case noFunctionAsVarSnippet.'
					);
				}
		...
		const withFunctionAsVarSnippet = provider
			...
			.then((items4) => {
				items4 =
					items4 instanceof vscode.CompletionList
						? items4.items
						: items4;
				const item4 = items4.find((x) => x.label === "funcAsVariable");
				if (!item4) {
					assert.fail(
						'Suggestion with label "Print" not found in test case withFunctionAsVarSnippet.'
					);
				}
				assert.equal(
		...
		const withFunctionAsVarSnippetNoType = provider
			...
			.then((items5) => {
				items5 =
					items5 instanceof vscode.CompletionList
						? items5.items
						: items5;
				const item5 = items5.find((x) => x.label === "funcAsVariable");
				if (!item5) {
					assert.fail(
						'Suggestion with label "Print" not found in test case withFunctionAsVarSnippetNoType.'
					);
				}
				assert.equal(
					(<vscode.SnippetString>item5.insertText).value,
					"funcAsVariable(${1:k})"
				);
			});
		const noFunctionAsTypeSnippet = provider
			...
			.then((items6) => {
				items6 =
					items6 instanceof vscode.CompletionList
						? items6.items
						: items6;
				const item1 = items6.find((x) => x.label === "HandlerFunc");
				const item2 = items6.find(
					(x) => x.label === "HandlerFuncWithArgNames"
				);
				const item3 = items6.find(
					(x) => x.label === "HandlerFuncNoReturnType"
				);
				if (!item1) {
					assert.fail(
						'Suggestion with label "HandlerFunc" not found in test case noFunctionAsTypeSnippet.'
					);
				}
		...
		const withFunctionAsTypeSnippet = provider
			...
			.then((items7) => {
				items7 =
					items7 instanceof vscode.CompletionList
						? items7.items
						: items7;
				const item11 = items7.find((x) => x.label === "HandlerFunc");
				const item21 = items7.find(
					(x) => x.label === "HandlerFuncWithArgNames"
				);
				const item31 = items7.find(
					(x) => x.label === "HandlerFuncNoReturnType"
				);
				if (!item11) {
					assert.fail(
	...
	test("Test No Completion Snippets For Functions", async () => {
		...
		const symbolFollowedByBrackets = provider
			...
			.then((items) => {
				...
				if (!item) {
					assert.fail(
						'Suggestion with label "Print" not found in test case symbolFollowedByBrackets.'
					);
				}
				assert.equal(
					!item.insertText,
					true,
					"Unexpected snippet when symbol is followed by ()."
				);
			});
		const symbolAsLastParameter = provider
			...
			.then((items1) => {
				...
				if (!item1) {
					assert.fail(
						'Suggestion with label "funcAsVariable" not found in test case symbolAsLastParameter.'
					);
				}
				assert.equal(
					!item1.insertText,
	...
	test("Test Completion on unimported packages (multiple)", async () => {
		...
		const items =
			completionResult instanceof vscode.CompletionList
		...
		expectedItems.forEach((expectedItem) => {
			...
			if (!actualItem.additionalTextEdits) {
				assert.fail(
					`Missing additionalTextEdits on suggestion for ${actualItem}`
				);
			}
			assert.equal(actualItem.additionalTextEdits.length, 1);
			assert.equal(
				actualItem.additionalTextEdits[0].newText,
				expectedItem.import
			);
		});
	});
				...
				.then((items) => {
					...
					assert.equal(
						...
						`expected number of completions: ${
							expected.length
						} Actual: ${labels.length} at position(${
							position.line + 1
	...
	test("goPlay - success run", async () => {
		...
		const goConfig = Object.create(
			vscode.workspace.getConfiguration("go"),
			{
				playground: {
					value: { run: true, openbrowser: false, share: false },
				},
			}
		);
	...
	test("goPlay - success run & share", async () => {
		...
		const goConfig = Object.create(
			...
			{
				playground: {
					value: { run: true, openbrowser: false, share: true },
				},
			}
		);
	...
	test("Build Tags checking", async () => {
		...
		const checkWithTags = check(
			...
			assert.equal(
				1,
				diagnostics.length,
				"check with buildtag failed. Unexpected errors found"
			);
			assert.equal(
				1,
		...
		const checkWithoutTags = check(
			...
			assert.equal(
				errMsg.includes(
					`can't load package: package test/testfixture/buildTags`
