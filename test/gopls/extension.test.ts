...
class Env {
	...
	constructor(projectDir: string) {
		...
		this.workspaceDir = path.resolve(
			projectDir,
			"test/gopls/testfixtures/src/workspace"
		);
		...
		if (this.workspaceDir !== workspaceFolder) {
			assert.fail(
				`specified workspaceDir: ${this.workspaceDir} does not match the workspace folder: ${workspaceFolder}`
			);
		}
	}
	...
	public async reset(fixtureDirName?: string) {
		// name of the fixtures subdirectory to use.
		try {
...
suite("Go Extension Tests With Gopls", function () {
	...
	test("HoverProvider", async () => {
		...
		const promises = testCases.map(
			async ([name, position, expectedSignature, expectedDoc]) => {
				...
				if (expectedSignature === null && expectedDoc === null) {
					assert.equal(
						hovers.length,
						0,
						`check hovers over ${name} failed: unexpected non-empty hover message.`
					);
					return;
				}
				...
				const gotMessage = (<vscode.MarkdownString>hover.contents[0])
					.value;
				assert.ok(
					gotMessage.includes(
						"```go\n" + expectedSignature + "\n```"
